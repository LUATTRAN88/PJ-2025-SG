...
class PAGE2:
    ...
    def create_layout(self,l1):
        ...
        third_thread = threading.Thread(target = self.loadingdata)
        third_thread.start()
            ...
            try:
                ...
                if response.status == 200:
                    ...
                    for i in self.rl_array:
                        ...
                        index+=1;
                    self.rl_power_array = self.origin_data['rl_power']
                    index2 = 0
                    for i in self.rl_power_array:
                        self.relay_power_objects[index2].setonoff(i["on"])
                        index2+=1;
                    pass
                else:
...
class SIGNAL_POWER:
    ...
    # def setonoff(self,val):
        # print(val)
        # if val == 1:
        #     img2 = ImageTk.PhotoImage(Image.open(get_path_img()+'lamp_signal_on.png'))
        #     self.btn_on.configure(image=img2)
        #     self.btn_on.image=img2
        # else:
        #     img3 = ImageTk.PhotoImage(Image.open(get_path_img()+'lamp_signal_off.png'))
        #     self.btn_on.configure(image=img3)
        #     self.btn_on.image=img3
